...
`num21 < num28` 使 `warpState` 降低后，下一帧确实可能因为 `warpExtra` 和 `exitDist` 同步降低，重新满足 `num21 >= num28`，使 `warpState` 增加。此时曲速退出不是固定 15 帧。

根因是退出判断使用当前 `warpState` 对应的 `exitDist`，而位置推进又会在接近退出边界时通过 `num29` 限制单帧曲速附加速度。若降低后的 `exitDist` 退到飞船剩余距离之后，下一帧会短暂回到加速分支。实测一维递推可出现 `-1/15` 与 `+1/60` 交替，曲速从 `warpState = 1` 降到 `0` 可能超过 15 帧，例如低曲速上限、靠近 `exitDist` 边界时可达到约 26 帧。

因此剩余时间算法不应把「曲速减速阶段」写死为 15 帧。推荐处理方式：

1. 在一维估算中逐帧递推 `warpState`、`exitDist`、`num29` 和受限后的 `warpExtra`，直到 `warpState <= 0`。
2. 每帧先用更新前的 `warpState` 判断加速或退出，再按代码更新 `warpState`，然后重新计算 `warpExtra` 并应用单帧防越界限制。
3. 若使用闭式近似，至少把退出时间视为近似量，并在文档或接口中标注可能低估；远距离排序影响较小，精确到秒的 UI 不宜使用。

上面的「曲速一维离散求和」应按这一规则实现，而不是在进入退出分支后强制连续扣 15 帧。
